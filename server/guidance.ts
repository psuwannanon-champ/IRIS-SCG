...
export async function runGuidance(req: GuidanceRequest, apiKey?: string) {
  ...
  const format = req.kind === 'diagnostic' ? diagnosticSchema : req.kind === 'coach' ? coachSchema : req.kind === 'clinic_briefing' ? briefingSchema : adviceSchema
  ...
  const response = await client.messages.parse({
    ...
    output_config: { format: zodOutputFormat(format), effort: req.kind === 'coach' ? 'medium' : 'high' },
  })
