...
| R58 | p10, p11 | The always-on coach serves every employee, not only cohort members | Expert Guidance falls back to the baseline, the self-paced path, invitations, BU priorities and the critical-skill catalogue when a person has no enrolment, and is told not to invent cohort dates, labs, clinics or a coach | Implemented |
| R54 | p10 | Personalised learning for every employee, not only cohort members | A learning plan can belong to the person: the org-wide baseline produces a self-paced path shown on the learning plan page | Implemented |
| R55 | p10, p5 04 | The path is re-personalised after every activity; telemetry feeds continuous re-personalisation | Re-personalise action rewrites what is still ahead from completed modules, labs, sprint evidence, coach questions and practice scores; completed and started modules are kept and the plan carries a revision | Implemented |
| R56 | p10 Input 02 | Learning history and AI-coach interactions feed the engine | Shared history card carries completed / in-progress / skipped modules, labs attended, recent coach questions and practice scores into every diagnostic and re-personalisation | Implemented |
| R57 | p4, p12 | The marketplace allocates key talent to roles, projects and gigs by verified skills | Posting owners search the workforce ranked by verified-skill match and invite a person; the invitation notifies them and they accept or decline | Implemented |
| R52 | p10 Input 01 | Role and level, KPIs, verified skills and gaps, interests, live projects, career aspiration and BU context feed the engine | All of them carried by the shared learner card into every personalisation context (diagnostic, baseline, weekly journey, contract review, coach, practice, re-personalisation); interests come from the person's marketplace postings with kind and status | Implemented |
