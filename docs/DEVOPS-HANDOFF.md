...
| Server-only secrets | `ANTHROPIC_API_KEY` (Vercel env, sensitive, production + preview) used by `api/guidance.ts`. Database password is used only to apply migrations. Never in `VITE_*`. |
...
| Health / smoke | `/login` renders; `/home` after choosing a persona; no amber *Local demo fixtures* badge; `POST /api/guidance` with `{"kind":"x"}` returns 400 `Unknown guidance kind` (function alive) and a real request returns JSON; create an evidence entry as Nara and confirm it appears after reload. |
