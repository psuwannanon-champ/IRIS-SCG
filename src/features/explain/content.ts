...
export const EXPLAINERS: Record<string, PageExplainer> = {
  ...
  blueprints: {
    ...
    functions: [{ id: 'bp', name: 'Role to capability plan', purpose: 'Describe a role the new operating model changes; the platform generates the critical future skills with target levels, three-year supply versus demand, THB at risk, a build / buy / borrow / bot call per gap, and the cohort plan.', benefit: 'Ready to implement: capability plans without weeks of co-design with management, which is the CBM turnaround promise.', users: 'Program office and BU sponsors.', roles: [{ role: 'Sponsor or program office', does: 'Describes the role and reviews the plan.' }, { role: 'Expert Guidance (Claude)', does: 'Maps the role to the taxonomy and quantifies the gaps from the BU agenda and verified supply.' }, { role: 'Program office', does: 'Adopts the plan, writing the gaps into the capability agenda.' }], stages: [{ icon: 'data', title: 'Data enters', body: 'Role title, level, headcount, operating-model change and responsibilities entered here; taxonomy, BU themes, existing agenda rows and verified passport supply come from the platform.' }, { icon: 'scales-01', title: 'Decision', body: 'Adopting creates capability-agenda rows awaiting a funded build / buy / borrow / bot decision.' }, { icon: 'arrow-right', title: 'What comes next', body: 'Funded build gaps drive cohort seats and assessment waves.' }], limitations: ['Supply figures are inferred from the demo dataset; production would read establishment data from the HR core.'] }],
  },
  agenda: {
